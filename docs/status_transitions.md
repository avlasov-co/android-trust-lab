...
Diff schema `2.6.0` keeps evidence status separate from observed values. A literal observed string such as `unknown` therefore remains data and cannot collide with the `not_collected`, `inaccessible`, `command_error`, or `unsupported` outcomes.
