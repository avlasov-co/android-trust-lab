...
json_string() { printf '"'; printf '%s' "$1" | sed 's/[\\"]/\\&/g'; printf '"'; }
...
verify_bundle() {
  ...
  while IFS='|' read -r rel size digest; do
    safe_artifact_path "$rel" && printf '%s\n' "$size" | grep -Eq '^[0-9]+$' && printf '%s\n' "$digest" | grep -Eq '^[0-9a-f]{64}$' || { rm -rf "$VERIFY_TMP"; return 1; }
    [ "$(grep -Fxc "$rel|$size|$digest" "$VERIFY_TMP/declared")" -eq 1 ] || { rm -rf "$VERIFY_TMP"; return 1; }
    file="$VERIFY_DIR/$rel"; [ -f "$file" ] && [ ! -L "$file" ] || { rm -rf "$VERIFY_TMP"; return 1; }
    [ "$(wc -c < "$file" 2>/dev/null | tr -d '[:space:]')" = "$size" ] && [ "$(sha256sum "$file" 2>/dev/null | awk '{print $1}')" = "$digest" ] || { rm -rf "$VERIFY_TMP"; return 1; }
    VERIFY_ARTIFACTS=$((VERIFY_ARTIFACTS + 1))
  done < "$VERIFY_TMP/declared"
  for file in $(find "$VERIFY_DIR" -type f -print 2>/dev/null); do
    rel=${file#"$VERIFY_DIR"/}; [ "$rel" = collector_manifest.json ] && continue
    awk -F'|' -v p="$rel" '$1 == p {ok=1} END {exit ok ? 0 : 1}' "$VERIFY_TMP/declared" || { rm -rf "$VERIFY_TMP"; return 1; }
  done
...
last_complete_id() {
  find "$OUT_DIR" -mindepth 1 -maxdepth 1 -type d -name 'run_*' -print 2>/dev/null | sort -r | while IFS= read -r dir; do
    [ -f "$dir/collector_manifest.json" ] && grep -Eq '^[[:space:]]*"completion_status": "complete",?$' "$dir/collector_manifest.json" || continue
    basename "$dir"; break
  done
}
...
op_verify() { bundle_path "$1" || { failure not_found 'The private collection was not found.'; return; }; if verify_bundle "$BUNDLE"; then success "{\"collectionId\":$(json_string "$1"),\"verified\":true,\"artifactCount\":$VERIFY_ARTIFACTS}"; elif [ "$VERIFY_CODE" = incomplete_collection ]; then failure incomplete_collection 'Only complete collections can be verified.'; else failure verification_failed 'The collection did not pass integrity verification.'; fi; }
...
op_export() {
  bundle_path "$1" || { failure not_found 'The private collection was not found.'; return; }; verify_bundle "$BUNDLE" || { failure "$VERIFY_CODE" 'The collection must be complete and pass verification before export.'; return; }
  [ -L "$EXPORT_PARENT" ] && { failure export_failed 'The fixed export location is unavailable.'; return; }; mkdir -p "$EXPORT_PARENT" 2>/dev/null || { failure export_failed 'The fixed export location is unavailable.'; return; }; dest="$EXPORT_PARENT/$1"; [ ! -e "$dest" ] || { failure export_exists 'An export with this collection ID already exists.'; return; }; tmp="$EXPORT_PARENT/.partial_$1_$$"; mkdir "$tmp" 2>/dev/null || { failure export_failed 'Could not prepare the export.'; return; }
  while IFS='|' read -r rel size hash; do target="$tmp/$rel"; mkdir -p "$(dirname "$target")" 2>/dev/null && cp "$BUNDLE/$rel" "$target" 2>/dev/null || { rm -rf "$tmp"; failure export_failed 'Could not copy the verified collection.'; return; }; done < "$VERIFY_TMP/declared"
  cp "$BUNDLE/collector_manifest.json" "$tmp/collector_manifest.json" 2>/dev/null && mv "$tmp" "$dest" 2>/dev/null || { rm -rf "$tmp"; failure export_failed 'Could not publish the export.'; return; }; success "{\"collectionId\":$(json_string "$1"),\"path\":$(json_string "$dest")}";
}
op_delete() {
...
case "$#:$1" in
  ...
  2:inspect) valid_id "$2" && op_inspect "$2" || failure invalid_id 'The collection ID is invalid.';;
  2:verify) valid_id "$2" && op_verify "$2" || failure invalid_id 'The collection ID is invalid.';;
  2:export) valid_id "$2" && op_export "$2" || failure invalid_id 'The collection ID is invalid.';;
  3:delete) valid_id "$2" && op_delete "$2" "$3" || failure invalid_id 'The collection ID is invalid.';;
